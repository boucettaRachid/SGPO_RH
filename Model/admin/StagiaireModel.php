...
class StagiaireModel {
    ...
    public function __construct() {
        ...
        public function insertStagiaire($name, $prenom, $cin, $address, $email, $tele, $username, $password, $ID_dep, $mission, $type_stage, $date_D, $date_F, $date_creation, $date_update,$image,$cv) {
            $statement = $this->PDO->prepare("INSERT INTO stagiaire (Name, Prenom, Cin, Address, Email, Tele, Username, Password, ID_dep, Mission, Type_stage, Date_D, Date_F, Date_creation, Date_update, Cv, Image) VALUES (:name, :prenom, :cin, :address, :email, :tele, :username, :password, :ID_dep, :mission, :type_stage, :date_D, :date_F, :date_creation, :date_update,:image,:cv)");
    ...
    public function updateStagiaire($ID_stagiaire, $name, $prenom, $cin, $address, $email, $tele, $username, $password, $ID_dep, $mission, $type_stage, $date_D, $date_F, $date_update,$image,$cv) {
        $statement = $this->PDO->prepare("UPDATE stagiaire SET Name = :name, Prenom = :prenom, Cin = :cin, Address = :address, Email = :email, Tele = :tele, Username = :username, Password = :password, ID_dep = :ID_dep, Mission = :mission, Type_stage = :type_stage, Date_D = :date_D, Date_F = :date_F, Date_update = :date_update ,Image = :image , Cv = :cv WHERE ID_stagiaire = :ID_stagiaire");
        $statement->bindParam(":name", $name);
        $statement->bindParam(":prenom", $prenom);
        $statement->bindParam(":cin", $cin);
        $statement->bindParam(":address", $address);
